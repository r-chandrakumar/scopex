...
import 'd_c_pick_view_page_model.dart';
export 'd_c_pick_view_page_model.dart';

class DCPickViewPageWidget extends StatefulWidget {
  const DCPickViewPageWidget({
    Key? key,
    this.id,
    required this.name,
  }) : super(key: key);
  ...
  @override
  _DCPickViewPageWidgetState createState() => _DCPickViewPageWidgetState();
}

class _DCPickViewPageWidgetState extends State<DCPickViewPageWidget> {
  late DCPickViewPageModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => DCPickViewPageModel());

    // On page load action.
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      if (FFAppState().accessToken != null && FFAppState().accessToken != '') {
        _model.dCViewRes =
            await InventoryApiGroupGroup.deliveryChallanViewCall.call(
          id: widget.id,
        ...
        if ((_model.dCViewRes?.succeeded ?? true)) {
          setState(() {
            _model.qty = InventoryApiGroupGroup.deliveryChallanViewCall
                .qtydone(
                                          ...
                                          Expanded(
                                            child: Container(
                                              ...
                                              child: GrnQtyTextBoxWidget(
                                                key: Key(
                                                    'Keybo9_${grnDetailssIndex}_of_${grnDetailss.length}'),
                                                id: getJsonField(
                                                  grnDetailssItem,
                                                  r'''$.id''',
                                                ),
